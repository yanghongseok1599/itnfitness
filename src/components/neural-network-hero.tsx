...
export default function NeuralNetworkHero({
  ...
  badgeLabel = "New",
  ctaButtons = [],
  microDetails = ["의학 석사", "52개 기관", "10년+ 경력"]
}: HeroProps) {
  ...
  return (
    <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden">
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16">
        <div ref={badgeRef} className="inline-flex items-center gap-2 rounded-full border border-purple-500/30 bg-purple-900/20 px-3 py-1.5 backdrop-blur-sm">
          <span className="text-[10px] font-light uppercase tracking-[0.08em] text-purple-300/70">{badgeLabel}</span>
          <span className="h-1 w-1 rounded-full bg-purple-400/60" />
          <span className="text-xs font-light tracking-tight text-purple-200/90">{badgeText}</span>
        </div>
        ...
        <div ref={ctaRef} className="flex flex-wrap items-center gap-3 pt-2">
          {ctaButtons.map((button, index) => (
            <a
              key={index}
              href={button.href}
              className={`rounded-2xl border border-purple-500/20 px-5 py-3 text-sm font-light tracking-tight transition-colors focus:outline-none focus:ring-2 focus:ring-purple-400/30 duration-300 ${
                button.primary
                  ? "bg-purple-600 text-white backdrop-blur-sm hover:bg-purple-700"
          ...
          {microDetails.map((detail, index) => {
            ...
            return (
              <li key={index} ref={refMap[index]} className="flex items-center gap-2">
                <span className="h-1 w-1 rounded-full bg-purple-400/60" /> {detail}
              </li>
            );
          })}
        </ul>
